...
def diffusion_train(dataloader_train, dataloader_test, feature_dataloader_tr, dataset_info, dset_info, args):
    ...
    '''
    torch.save(generated_features,'tensor.pt')
    torch.save(diffusion_model.state_dict(),'diffusion_model_params.pth')
    '''
...
def dm_sample(dataloader_train, dataloader_test, feature_dataloader_tr, dataset_info, dset_info, args):
    # Create the model and optimizer
    # seed = 123
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # scaler = torch.cuda.amp.GradScaler()
    ...
    diffusion_model = torch.load("F:\\codes\\FLDM\\fldm_CIFAR10_1225_round_110_fid_3.9039533334302505.pth",weights_only=False)
