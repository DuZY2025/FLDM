...
def evaluate_loop(dataloader, model, loss_fn, device, dset_info, epoch):
    ...
    _, mmf_acc = get_metrics(probs, labels, dset_info['per_class_img_num'], epoch)
    ...
    label_shift_acc, mmf_acc_pc = get_metrics(pc_probs, labels, dset_info['per_class_img_num'], epoch)
...
def get_metrics(probs, labels, cls_num_list, epoch):
...
def fine_tune_fc(generated_features, fake_classes, feature_dataset_tr, test_set, dataset_info, args, dset_info, cfg):
    ...
    for epoch in range(args.epoch):
        ...
        valid_loss, valid_accuracy, label_shift_acc, mmf_acc, mmf_acc_pc  = evaluate_loop(test_set, model_totrain, loss_fn, device, dset_info, epoch)# fixme
