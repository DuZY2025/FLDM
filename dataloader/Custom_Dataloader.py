...
def data_loader_wrapper_cust(cust_dataset):
    ...
    if cust_dataset["name"] in ['CIFAR10', 'CIFAR100']:
        ...
        # transforms.Compose定义处理图像的流水线
        # transforms.RandomHorizontalFlip()以一定概率水平翻转图片，不是旋转，默认概率0.5
        # transforms.RandomCrop(32, 4) 随机裁剪，size32*32，padding 4 边缘填充4个像素
        # ToTensor() 通常放在最后，因为它会将图像转换为张量格式
        data_transforms = {
