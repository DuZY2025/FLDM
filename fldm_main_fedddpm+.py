...
for i in range(args.num_rounds+1):
    ...
    for client in online_clients:
        ...
        for epoch in range(client_epochs):
            ...
            for feature, feature_label in dataloader_phase1[client]:
                ...
                loss, t = clientDM(feature, feature_label)
                loss.backward()
                opt_phase1.step()
    ...
    # 客户端样本总数
    total_samples = sum(img_num_per_class)
    
    # 1. 按样本数加权聚合可训练参数
    for name, param in globalDM.named_parameters():
        weighted_param = 0.0
        for j in range(len(local_models)):
            local_param = local_models[j].state_dict()[name]
            weight = len(list_client_indices[online_clients[j]]) / total_samples
            weighted_param += weight * local_param
        new_state[name] = weighted_param
    
    # 2. buffers（如 BatchNorm 的 running_mean / var）不做聚合
    for name, buf in globalDM.named_buffers():
        new_state[name] = buf
